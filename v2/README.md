...
A dedicated bare-metal firewall replacing v1's virtualized-Sophos-on-Proxmox design, with real VLAN segmentation, dual-purpose VPN egress, and (soon) a Proxmox host and NAS behind it. This document is a living record, updated as the build progresses — not a retrospective like v1.
...
**DNS/DHCP:** split design — Unbound for recursive resolution + DNSSEC + DNSBL, dnsmasq for DHCP and authoritative local DNS (`lab.lan`) on a non-standard port, with an Unbound domain override tying them together. Each VLAN interface gets its own DHCP range and its own entry in Unbound's listener list — a VLAN with an IP but no DNS resolution is the most common way a segmented network looks "broken" when it's actually just a missed checkbox.
...
**Deployed:**
- Topton mini PC (Intel J6412, 4× i226-V) — OPNsense 26.7.1, FreeBSD 15.1, ZFS root.
- Ubiquiti Switch Lite 16 PoE (8 of 16 ports PoE+) — layer 2, awaiting controller adoption.
...
**Software services (planned):** media server (Jellyfin or similar, storage on NAS via NFS, app on Proxmox), UniFi Network Application, and whatever else emerges — deliberately left open rather than over-planned.
